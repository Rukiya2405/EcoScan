...
class _PostDetailScreenState extends State<PostDetailScreen> {
  ...
  // ============================================================
  // DELETE POST + DELETE IMAGE FROM STORAGE
  // ============================================================
    ...
    try {
      ...
      if (imageUrl != null && imageUrl.isNotEmpty) {
        try {
          ...
          // DON'T STOP IF IMAGE DELETE FAILS
          debugPrint('Image delete error (non-fatal): $e');
        }
      }

      // DELETE ALL COMMENTS FIRST
      final QuerySnapshot comments = await FirebaseFirestore.instance
          .collection('posts')
          .doc(widget.postId)
          .collection('comments')
          .get();
      ...
      for (final DocumentSnapshot doc in comments.docs) {
  ...
  Future<void> _submitComment() async {
    ...
    try {
      ...
      final String postAuthorId = widget.data['authorId'] ?? '';
  ...
  Future<void> _toggleLike(List likes) async {
    ...
    if (likes.contains(currentUid)) {
      ...
      await NotificationService.sendLikeNotification(
        postAuthorId: postAuthorId,
        likerName: likerName,
        postContent: widget.data['content'] ?? '',
        postId: widget.postId,
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    final String name = widget.data['authorName'] ?? 'User';
    final String content = widget.data['content'] ?? '';
    final String tag = widget.data['tag'] ?? 'General';
    final String? imageUrl = widget.data['imageUrl'] as String?;
    final bool isOwner = widget.data['authorId'] == currentUid;
